...
public class ImageSliderAdapter extends PagerAdapter implements ViewPager.OnPageChangeListener {
    ...
    public Object instantiateItem(@NonNull ViewGroup view, int position) {
        View myImageLayout = LayoutInflater.from(context).inflate(R.layout.item_slider, view, false);
        ImageView myImage = myImageLayout.findViewById(R.id.image);
        if(crop){
            Glide.with(context).load(items.get(position).getItem()).transition(DrawableTransitionOptions.withCrossFade()).apply(RequestOptions.centerCropTransform()).into(myImage);
        }
        else{
            Glide.with(context).load(items.get(position).getItem()).transition(DrawableTransitionOptions.withCrossFade()).into(myImage);
        }
